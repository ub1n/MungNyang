...
class RecordChangeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        record.rec=rec
        record.image=image
        rec_name.setText(record.name)
        rec_editText.setText(record.rec)
        val byteArray=record.image
        if(record.image!=null) {
            val picture = BitmapFactory.decodeByteArray(byteArray, 0, byteArray!!.size)
            rec_pic.setImageBitmap(picture)
        }
        rec_add.setOnClickListener{
            ...
            Thread { database?.recordDao?.update(record) }.start()
